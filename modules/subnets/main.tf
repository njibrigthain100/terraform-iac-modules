...
resource "aws_subnet" "private_subnet" {
  ...
  for_each          = { for idx, az in var.subnets.az : idx => { az = az, cidr = var.subnets.private_subnets_cidr[idx] } }
...
resource "aws_subnet" "public_subnet" {
  vpc_id = var.subnets.vpc_id
  # The for_each argument here iterates over the az and subnet and sets their values and sets theindex(idx) as the key
  for_each                = { for idx, az in var.subnets.az : idx => { az = az, cidr = var.subnets.public_subnets_cidr[idx] } }
  availability_zone       = each.value.az
  cidr_block              = each.value.cidr
